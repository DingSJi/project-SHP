        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <!-- 进行路由跳转之前要向服务器发请求，把购买的信息给服务器进行存储 -->
...
export default {
  ...
  methods: {
    ...
    //加入购物车函数
    async addShopCart() {
      // 流程：发请求 将产品加入到数据库 服务器存储成功 进行路由跳转 失败 给用户提示
      // detail/addOrUpdateShopCart 返回一个promise对象，用await接收，再用try catch进行判断
      try {
        await this.$store.dispatch("detail/addOrUpdateShopCart", {
          skuId: this.$route.params.skuid,
          skuNum: this.skuNum,
        });
        // 成功 进行路由跳转（并将产品信息(skuNum,skuInfo)带给跳转的路由）
